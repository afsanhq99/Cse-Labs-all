...
def writeRecord():
    ...
    recordFile.writelines(
        f"Percentage of odd parity: {((odd / (odd + even + noParity)) * 100)}%\n"
    )
    recordFile.writelines(
        f"Percentage of even parity: {((even / (odd + even + noParity)) * 100)}%\n"
    )
    recordFile.writelines(
        f"Percentage of no parity: {((noParity / (odd + even + noParity)) * 100)}%\n"
    )
    recordFile.writelines(
        f"Percentage of palindrome: {(palindrome / (palindrome + nonPalindrome)) * 100}%\n"
    )
    recordFile.writelines(
        f"Percentage of palindrome: {(nonPalindrome / (palindrome + nonPalindrome)) * 100}%\n"
    )
    recordFile.close()
